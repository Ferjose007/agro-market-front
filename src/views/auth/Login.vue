...
const handleLogin = async () => {
  ...
  try {
    ...
    // 1. GESTIÓN DE ESTADO (De tu rama HEAD)
    // Usamos el store de Pinia, no localStorage directo
    auth.setAuthData(response.data.access_token, response.data.user);

    // 2. REDIRECCIÓN INTELIGENTE (De tu rama HEAD)
    ...
    // 3. MANEJO DE ERRORES (De la rama de Raul)
    // Agregamos feedback visual que faltaba en tu rama
    console.error(error); // Un log nunca está de más para ti como dev
  ...
  <div class="min-h-screen flex bg-white font-sans">
      ...
      <div class="w-full max-w-lg space-y-8">
        ...
        <form class="mt-8 space-y-6" @submit.prevent="handleLogin">
          <div class="space-y-5">
            ...
            <div>
              ...
              <div class="flex justify-end mt-2">
                <a href="#" class="text-sm font-medium text-agro-primary hover:text-agro-primary-dark">¿Olvidaste tu
                  contraseña?</a>
              </div>
            </div>
          </div>
          ...
          <div v-if="errorMessage"
            class="bg-red-50 border border-red-100 text-red-600 p-4 rounded-xl text-sm text-center flex items-center justify-center gap-2">
    ...
    <div class="hidden lg:block lg:w-1/2 relative">
      ...
      <div class="absolute bottom-0 left-0 p-12 text-white">
        <blockquote class="text-3xl font-bold font-serif leading-tight">"Cultivando futuro, <br />cosechando confianza."
        </blockquote>
      </div>
    </div>
  </div>
